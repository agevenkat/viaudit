...
export async function queryCopilot(params: EngineQueryParams): Promise<EngineResult | null> {
  ...
  return withRetry(async () => {
    const completion = await getClient().chat.completions.create({
      ...
      messages: [
        {
          role:    'system',
          content: 'You are Microsoft Copilot. Provide helpful, accurate answers with web sources. When recommending products or tools, cite specific sources and provide balanced comparisons.',
        },
    ...
    if (!response) {
      logger.warn({ prompt }, 'copilot: empty response');
